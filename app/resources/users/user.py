...
from app.resources.users.user_docs import user_in_path_doc, user_put_doc
from app.utils.archive_manager import archive_user
...
class User(Resource):
    ...
    def delete(self, token_user_id, user_id):
        try:
            if token_user_id != user_id:
                return make_response("Insufficient Permissions", 403)
            user_obj = UserModel.objects.get(id=ObjectId(user_id))
            archive_user(user_obj)

            return jsonify(archived_user=user_id)
        except InvalidId:
            return make_response("Invalid user ID", 400)
        except JSONDecodeError as e:
            return make_response("Invalid JSON: {}".format(e.__str__()), 400)
